...
- **A FreePlane `.mm` format specification** (`docs/FREEPLANE_MM_FORMAT_SPEC.md`), reverse-engineered from the FreePlane source (`MapWriter`, `NodeWriter`, `NodeTextBuilder`, `TreeXmlWriter`/`XMLWriter`). It records where FreePlane differs from FreeMind — UTF-8 output rather than numeric-entity ASCII, insertion-order rather than alphabetical attributes, an always-on `ID` per node, an explicit root `FOLDED="false"`, and 4-space pretty-printing.
- **The complete FreePlane online map gallery as test fixtures.** Eleven real maps from freeplane.org (`WhatIsMindMapping`, `freeplaneApplications`, `freeplaneFunctions`, `freeplaneTutorial`, `CollectionBeginner`, `CollectionAdvanced`, `MeetingBeginner`, `MeetingAdvanced`, `Action-dashboard`, `SWOT`, `Vault`), spanning format versions 0.9.0 through `freeplane 1.2.0` and sizes from 700 B to 214 KB, committed under `utils/__tests__/fixtures/` and imported by the compatibility suite.
- **A CI workflow** (`.github/workflows/ci.yml`) that runs on every pull request and push to `main`. A frontend job type-checks, runs the full vitest suite (which also covers `packages/*`), runs the import/export round-trip gate, and checks offline parity; a desktop job builds the frontend and runs `cargo check` and `cargo test` on the Rust host. Previously no workflow ran the tests at all.
...
- **The browser demos crashed when opening Settings.** `demo/` and `mobile-demo/` render the editor with no `<Router>`, but the Settings modal's Account tab called `useNavigate()` unconditionally, so clicking the gear threw `useNavigate() may be used only in the context of a <Router> component.` and blanked the page. The hook is now resolved only when a router is present (`useInRouterContext`), and the one navigation — "Done → /vaults" after a password change — is skipped where there is no router. The desktop app was never affected: `AppRoot` wraps everything in `<BrowserRouter>`. `components/SettingsModal.tsx`.
